...
public sealed class MapPreviewService : IMapPreviewService
{
    ...
    public async Task<MapPreviewData> LoadMapPreviewAsync(
        InstanceProfile profile,
        string? saveFilePath = null,
        CancellationToken cancellationToken = default)
    {
        var result = await LoadMapPreviewCoreAsync(profile, saveFilePath, cancellationToken);
        if (!result.IsSuccess || result.Value is null)
            throw new InvalidOperationException(result.Message ?? "读取地图预览失败。", result.Exception);

        return result.Value;
    }

    private async Task<OperationResult<MapPreviewData>> LoadMapPreviewCoreAsync(
        InstanceProfile profile,
        string? saveFilePath,
    ...
    private static BlockColorTable BuildBlockColorTable(InstanceProfile profile, Dictionary<int, string> blockCodeById)
    {
    ...
    private static BlockMetadataResolver? BuildBlockMetadataResolver(InstanceProfile profile)
    {
        try
        {
            var installPath = WorkspacePathHelper.GetServerInstallPath(profile.Version);
            var cacheKey = BuildRuleCacheKey(installPath, profile.DirectoryPath);
            ...
            var rules = LoadRules(installPath, profile.DirectoryPath);
            lock (RuleCacheGate)
            {
    ...
    private static string ResolveSavePath(InstanceProfile profile, string? saveFilePath)
    {
